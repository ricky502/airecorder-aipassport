...
每个应用在其贡献者文件夹下拥有独立子目录。用户名文件夹按原样保留（大小写不敏感，与 GitHub
用户名一致），应用文件夹用英文小写连字符命名。仅在应用发布或准备记录时建档，不要预先创建空骨架。
...
`<username>` 是贡献者的 GitHub 用户名（大小写不敏感，按用户名原样保留，如 `Shinku-Chen/`）。
...
下的官方产品图，为屏幕内容加上机器外壳：保持外壳、按键、接口与钥匙扣孔与这些参考一致，
让渲染出的屏幕画面像出现在真实 AI Passport 设备里，而不是一块裸屏幕。
