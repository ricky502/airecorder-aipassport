...
这些图是 AI Passport 市场宣传与展示物料的**视觉基线**。可用 AI 图像工具基于这些参考图生成效果图，生成时保持产品轮廓、屏显布局、Logo 文字与配色与参考一致；新生成的渲染图请登记进 [`assets/brand/README.md`](assets/brand/README.md) 索引。素材作为内部视觉基线使用，除非参考图本身允许复用，否则不要原样搬运到公开发布物料中。
