...
- 修复录音列表删除竞态：上传线程已暂时接管的本地录音仍可从列表删除，并同步清理队列状态。

- 修复 AI Passport 后端对滚动录音分片的解析：一个分片内拼接的多个 IMA-ADPCM 小包现在会完整合并为 WAV；新增飞书 Agent Webhook 的 JSON 投递、Bearer 认证、幂等键和重试机制。只有目标确认接收后才标记完成，未配置或失败时保留本地音频。
